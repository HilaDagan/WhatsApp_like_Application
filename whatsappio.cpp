...
void parse_command(const std::string& command, command_type& commandT, 
    ...
    if(!strcmp(s, "create_group")) {
        ...
        if(!s) {
            ...
            name = s;
            if(name.size()+5 == command.size()) {// there is no space after the group name
                commandT = INVALID;
                return;
            }
...
bool isLettersOrDigits(const std::string &input){
    for (const char c: input)
    {
        if (!isalpha(c) && !isdigit(c)){
            return false;  // character is not alphabetic and not digit
        }
    }
    return true;  // Every character is alphabetic or digit
}
